...
export function useChat() {
  ...
  useEffect(() => {
    ...
    const wsUrl = `${API_ROUTES.COMMUNICATIONS.replace('http', 'ws')}/ws-api/v1/comms`;
    ...
    stompClient.current.onConnect = (frame) => {
      ...
      stompClient.current.subscribe('/topic/support/' + clientIdRef.current, (message) => {
  ...
  const sendMessage = (message) => {
    ...
    stompClient.current.publish({
      destination: '/relatosdepapel/support/message',
