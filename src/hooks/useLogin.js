...
export function useLogin() {
  ...
  const { user, setUser, setAccessToken, clearSession, startSessionTimer } =
  ...
  const login = async (username, password) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userInfoUrl = buildApiUrl("USERS", `/api/v1/users/me`);
  ...
  const renewSession = async (currentAccessToken) => {
    try {
      ...
      const renewalUrl = buildApiUrl(
        "USERS",
        `/api/v1/tokens/${currentAccessToken}/renewals`,
      );
